...
<body> 
   ...
   <section id="main">
    ...
    $id = $_POST['id'];
    $sql = "select * from nhanviens where NV_ID = '".$id."' ";
    $res = $conn->query($sql);
    $row = $res->fetch_assoc();
  ?>
...
<div class="col-5" style="margin-left: auto; margin-right: auto;">
            <strong><b>SỬA NHÂN VIÊN</b></strong>
            <form class="mt-3" action="suaNV.php" method="post">
                <div class="col-7">
                    ID: <input type="text" name="id" value="<?php echo $row['NV_ID'] ?>" >
                </div>
                <div class="col-3">
                ...
                <select name="CV_ID" class="form-control mb-3">
                    <?php
                        while($rowcv = mysqli_fetch_assoc($query_cv)){?>
                        <option value="<?php echo $rowcv['CV_ID']; ?>"> <?php echo $rowcv['CV_TEN']; ?> </option>
                        <?php } ?>
                </select>
            </div>
            <div class="col-7">
                ...
                <!-- <input type="text" class="form-control" name=""> -->
                <input type="text" name="NV_HOTEN" id="NV_HOTEN" class="form-control" required value="<?php echo $row['NV_HOTEN']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">NGÀY SINH</label>
                <input type="date" class="form-control" name="NV_NGAYSINH" required value="<?php echo $row['NV_NGAYSINH']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">GIỚI TÍNH</label>
                <input type="text" class="form-control" name="NV_GIOITINH" value="<?php echo $row['NV_GIOITINH']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">QUÊ QUÁN</label>
                <input type="text" name="NV_QUEQUAN" class="form-control" value="<?php echo $row['NV_QUEQUAN']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">SỐ ĐIỆN THOẠI</label>
                <input type="text" class="form-control" name="NV_SDT" required value="<?php echo $row['NV_SDT']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">EMAIL</label>
                <input type="text" class="form-control" name="NV_EMAIL" required value="<?php echo $row['NV_EMAIL']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">CCCD</label>
                <input type="text" class="form-control" name="NV_CCCD" required value="<?php echo $row['NV_CCCD']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">TÀI KHOẢN NHÂN VIÊN</label>
                <input type="text" class="form-control" name="NV_TAIKHOAN" required value="<?php echo $row['NV_TAIKHOAN']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">MẬT KHẨU NHÂN VIÊN</label>
                <input type="text" class="form-control" name="NV_MATKHAU" required value="<?php echo $row['NV_MATKHAU']; ?>">
            </div>
            ...
            <div class="col-7 mt-3">
                <button name="sbm" class="btn btn-success" type="submit">SỬA NHÂN VIÊN</button>
            </div>
