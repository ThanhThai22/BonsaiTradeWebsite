...
<body> 
  ...
  <header id="header">
      ...
      <div class="row">
        <div class="col-md-3">
          <div class="list-group mt-5">
            ...
            <a href="viewSP.php" class="list-group-item p-3"><span class="" aria-hidden="true"></span> DANH SÁCH SẢN PHẨM </a>  
    ...
    $sql_lsp = "select * from loaisanphams";
    $query_nv = $conn->query($sql_nv);
    $query_lsp = $conn->query($sql_lsp);
...
<div class="col-5" style="margin-left: auto; margin-right: auto;">
            <strong><b>SỬA SẢN PHẨM</b></strong>
            <form action="suaSP.php" method="post">
            <div class="col-3">
                    ID: <input type="text" name="id" value="<?php echo $row['SP_ID'] ?>" >
                </div>
            <div class="col-7">
                <label for="" class="">NHÂN VIÊN</label>
                <!-- <input type="text" class="form-control" name=""> -->
                <select class="form-select" aria-label="Default select example" name="nv_id" class="form-control">
                    <?php
                        while($rownv = mysqli_fetch_assoc($query_nv)){?>
                        <option value="<?php echo $rownv['NV_ID']; ?>"> <?php echo $rownv['NV_HOTEN']; ?> </option>
                        <?php } ?>
                </select>
            </div>
            <div class="col-7">
                ...
                <!-- <input type="text" class="form-control" name=""> -->
                <select class="form-select" aria-label="Default select example" name="lsp_id" class="form-control">
                    <?php
                        while($rowlsp = mysqli_fetch_assoc($query_lsp)){?>
                        <option value="<?php echo $rowlsp['LSP_ID']; ?>"> <?php echo $rowlsp['LSP_TEN']; ?> </option>
                        <?php } ?>
                </select>
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">TÊN SẢN PHẨM</label>
                <input type="text" class="form-control" name="sp_ten" required value="<?php echo $row['SP_TEN']; ?>" >
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">MÔ TẢ</label>
                <input type="text" class="form-control" name="sp_mota" value="<?php echo $row['SP_MOTA']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">HÌNH ẢNH</label>
                <input type="text" name="sp_linkanh" class="form-control" value="<?php echo $row['SP_LINKANH']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">ĐƠN GIÁ</label>
                <input type="text" class="form-control" name="sp_dongia"required value="<?php echo $row['SP_DONGIA']; ?>">
            </div>
