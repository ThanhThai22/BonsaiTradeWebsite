...
<body> 
   ...
   <section id="main">
    ...
    $id = $_POST['id'];
    $sql = "select * from dichvus where DV_ID = '".$id."' ";
    $res = $conn->query($sql);
    $row = $res->fetch_assoc();
  ?>
...
<strong><b>SỬA DỊCH VỤ</b></strong>
            <form class="mt-3" action="" method="post">
            <div class="col-3">
                    ID: <input type="text" name="id" value="<?php echo $row['DV_ID'] ?>" >
                </div>
                <div class="col-7">
                <label for="" class="">TÊN DỊCH VỤ</label>
                <!-- <input type="text" class="form-control" name=""> -->
                <input type="text" name="DV_TEN" class="form-control" required value="<?php echo $row['DV_TEN']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">MÔ TẢ</label>
                <input type="text" class="form-control" name="DV_MOTA" required value="<?php echo $row['DV_MOTA']; ?>">
            </div>
            <div class="col-7">
                <label for="" class="col-form-label">ĐƠN GIÁ</label>
                <input type="text" class="form-control" name="DV_DONGIA" required value="<?php echo $row['DV_DONGIA']; ?>">
            </div>
            ...
            <div class="col-3 mt-3">
                <button name="btnSave" class="btn btn-success" type="submit">SỬA CSDL</button>
            </div>
